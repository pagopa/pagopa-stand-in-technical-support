...
public class SupportService {
  ...
  public ResponseContainer getEvents(Optional<String> station, LocalDate from, LocalDate to) {
    ...
    List<CosmosEventModel> collect =
        cosmosEventsRepository.find(station, dateRequest.getFrom(), dateRequest.getTo()).stream()
            .map(
                ee -> {
  ...
  public List<StandInStation> getStations() {
    return standInStationsRepository.getStations().stream()
        .map(
            s ->
                StandInStation.builder()
  ...
  private DateRequest verifyDate(LocalDate dateFrom, LocalDate dateTo) {
    if (dateFrom == null && dateTo != null || dateFrom != null && dateTo == null) {
      throw new AppException(AppError.BAD_REQUEST, "Date from and date to must be both defined");
    } else if (dateFrom != null && dateTo != null && dateFrom.isAfter(dateTo)) {
      throw new AppException(AppError.BAD_REQUEST, "Date from must be before date to");
    }
    ...
    if (ChronoUnit.DAYS.between(dateFrom, dateTo) > dateRangeLimit) {
      throw new AppException(AppError.INTERVAL_TOO_LARGE, dateRangeLimit);
    }
